...
fs.readdir(directory_pic, function (err, files) {
    ...
    for(file in files){
        ...
        cmd = `scp -r ../../avatares/${files[file]} aker-service:/var/www/files.aker.network/html/pic_profile/${files[file]}`;
...
fs.readFile('./student.csv', 'utf8', function (err, data) {
  ...
  for(name in data){
      ...
      cmd = `npm run aker-gql-op -- user_student_group_add --full_name "${full_name}"  --pic_path "https://files.aker.network/pic_profile/${array[4]}" --group_id ${group_id} --inventory_group_id ${inventory_group_id} --user_id ${user_id}`;
