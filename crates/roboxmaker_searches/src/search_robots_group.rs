...
impl Component for SearchRobotdGroup {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        let on_focus = ctx.link().callback(move |_| SearchRobotdGroupMessage::OnFocus);
        let on_blur = ctx.link().callback(move |_| SearchRobotdGroupMessage::OnBlur);
        let on_hidden_modal = ctx.link().callback(move |_| SearchRobotdGroupMessage::HiddenModal);

        let on_search = ctx.link().callback(|search: InputEvent| SearchRobotdGroupMessage::FetchRobotsByRobotsGrade(get_value_from_input_event(search)));
