...
impl Component for DegreeContentStudent {
    ...
    fn create(ctx: &Context<Self>) -> Self {
        ctx.link().send_message(DegreeContentStudentMessage::FetchClassGroups);
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        let class_group_members = |_class_group: &ClassProfile| {
            html! {
                // <UserList user_profile=ctx.props().user_profile.clone() group_id=class_group.group_id
                //     on_app_route=ctx.props().on_app_route.clone()
                //     class_name=class_group.class_name.clone()
                //     saved_sidebar_state=ctx.props().saved_sidebar_state.clone() />
            }
        };
        ...
        let class_group_level = |class_group: &ClassProfile| {
            ...
            let class_group_category_desktop = |&category| {
                ...
                let on_click_category = Callback::from(move |_| navigator.push(&AppRoute::GroupSectionStudent{school_id, user_id, category}));

                // let on_click_category = ctx.link().callback(move |_| 
            ...
            // let on_schools_staff = ctx.link().callback(move |_| DegreeContentStudentMessage::AppRouteChanged(AppRoute::GradesBySchoolId{school_id}));
            // let on_schools_teacher = ctx.link().callback(move |_| DegreeContentStudentMessage::AppRouteChanged(AppRoute::GradesByUserId{school_id}));
